...
export default {
  ...
  methods: {
    ...
    pdf() {
      ...
      if(vm.rowDataTable.length > 0) {
        headerTable += '' +
          ...
          '<td style="border: 1px solid #ddd;">T_MANTENIBLE</td>' +
          ...
          '<td style="border: 1px solid #ddd;">RENDIMIENTO (KM / L)</td>' +
          '<td style="border: 1px solid #ddd;">DESC_MANTENIBLE</td>'
        ...
        for(var x = 0; x < vm.rowDataTable.length; x++) {
          detalle += '<tr style="border: 1px solid #ddd;">' +
            ...
            '<td style="border: 1px solid #ddd;">' + ((vm.rowDataTable[x].distancia / vm.rowDataTable[x].litros ).toFixed(2)) + '</td>' +
        ...
        for (let c = 0; c < this.Temporal.length; c++) {
          ...
          if (this.rowData[b].sucursal_id === this.Temporal[c].sucursal_id && this.Temporal[c].sucursal_id !== 0) {
            ...
            if (this.agrupar == true) {
              ...
              this.rowData[b].rendimiento = 0.0;
              this.Temporal[c].costo += this.rowData[b].costo
              this.rowData[b].rendimiento = (this.rowData[b].distancia / this.rowData[b].litros).toFixed(2)
               
            } else {
              this.rowData[b].rendimiento = 0.0;
              var costoSumar = parseFloat(this.rowData[b].distancia / this.rowData[b].litros ).toFixed(2)
              this.Temporal[c].costo += costoSumar
              var listrosSumar = parseFloat(this.rowData[b].litros.replace('$',''))
              this.rowData[b].rendimiento =  costoSumar; // (distancia / listrosSumar).toFixed(2)
    ...
    getPie () {
      ...
      for (let d = 0; d < this.Temporal.length; d++) {
        this.pieLabel.push(this.Temporal[d].sucursal_nombre)
        this.pieData.push((this.Temporal[d].distancia / this.Temporal[d].litros).toFixed(2))
        this.dataColor.push(this.getRandomColor())
      }
      /* Actualizando en arreglo de gráfica */
      ...
      for (let b = 0; b < this.rowData.length; b++) {
        for (let c = 0; c < this.Temporal.length; c++) {
          if (this.rowData[b].sucursal_id === this.Temporal[c].sucursal_id) {
            ...
            if (this.agrupar == true) {
              this.Temporal[c].costo += parseFloat(this.rowData[b].litros / this.rowData[b].distancia )
            } else {
               
              //ORIGINAL
              //var costoSumar = parseFloat(this.rowData[b].costo.replace('$','').replace(',',''))
              var costoSumar = parseFloat(  this.rowData[b].distancia / this.rowData[b].litros  ).toFixed(2)
              //litros
              //distancia
               
              this.Temporal[c].costo += costoSumar;
              ///console.log(this.Temporal[c].costo.toFixed(2)) 
            }
          }
        }
      }
      ...
      for (let b = 0; b < this.rowData.length; b++) {
        for (let c = 0; c < this.Temporal.length; c++) {
          if (this.rowData[b].mantenible_id === this.Temporal[c].mantenible_id) {
            if (this.agrupar == true) {
              this.Temporal[c].costo += (this.rowData[b].distancia / this.rowData[b].litros).toFixed(2)
            } else {
              var costoSumar = parseFloat(this.rowData[b].distancia / this.rowData[b].litros ).toFixed(2)
              this.Temporal[c].costo += costoSumar
            }
    ...
    getDataArr () {
      ...
      // Recupera sucursales
      unirest('POST', 'http://localhost:51104//api/Reportes/ConsultarSucursales')
      ...
      unirest('POST', 'http://localhost:51104//api/Reportes/ConsultarTipoVehiculos')
        .send({ 'EmpresaID': vm.getEmpresa() })
        .end(function (res) {
          if (res.error) {
            vm.text = 'Usuario o Contraseña no válidos intente con otras credenciales'
      ...
      // Recupera tipo de vehículos
      unirest('POST', 'http://localhost:51104//api/Reportes/ConsultarMantenibles')
        .send({ 'EmpresaID': vm.getEmpresa() })
        .end(function (res) {
          if (res.error) {
            vm.text = 'Usuario o Contraseña no válidos intente con otras credenciales'
    ...
    getGraficaData () {
      ...
      if (vm.consultar != null) {
        ...
        if (vm.consultar.tipo == 1) {
          ...
          vm.overlay = true
          unirest('POST', 'http://localhost:51104//api/Reportes/ConsultarRendimientoCombustible')
            .send({
              'EmpresaID': empresa,
              'sucursal_id': [ datos ],
    ...
    SumDataArr(arr) {
      ...
      for (var b = 0; b < areglo.length; b++) {
        for (var c = 0; c < arr.length; c++) {
          if (areglo[b].mantenible_id == arr[c].mantenible_id) {
            ...
            areglo[b].rendimiento += parseFloat(arr[c].distancia / arr[c].litros).toFixed(2)
      ...
      for (var d = 0; d < areglo.length; d++) {
        ...
        areglo[d].rendimiento = parseFloat(areglo[d].distancia / areglo[d].litros).toFixed(2)
    ...
    FormatearDatos(arr) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        arr[i].rendimiento = parseFloat(arr[i].distancia / arr[i].litros).toFixed(2)
  ...
  beforeMount () {
    this.colsTable = [
      ...
      { text: 'RENDIMIENTO (KM/L)', value: 'rendimiento', width: 100 },
