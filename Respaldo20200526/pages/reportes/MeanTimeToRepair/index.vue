  ...
  methods: {
    ...
    getPieMounted () {
      ...
      for (let b = 0; b < this.rowData.length; b++) {
        ...
        // eslint-disable-next-line new-cap,no-undef,camelcase,no-unused-vars
        const tiempo_reparacion = new Date(this.rowData[b].fecha_fin).getTime() - new Date(this.rowData[b].fecha_solicitud).getTime()
        // eslint-disable-next-line camelcase
    ...
    getGraficaData () {
      ...
      if (vm.consultar != null) {
        ...
        if (vm.consultar.tipo == 1) {
          ...
          vm.overlay = true
          unirest('POST', 'http://localhost:51104//api/Reportes/ConsultarReporteMeanTimeToRepair')
            .send({
              'EmpresaID': empresa,
              'sucursal_id': [ datos ],
      ...
      for (var b = 0; b < areglo.length; b++) {
        for (var c = 0; c < arr.length; c++) {
          if (areglo[b].mantenible_id == arr[c].mantenible_id) {
            ...
            areglo[b].tiempo_reparacion += (arr[c].tiempo_reparacion != null) ? arr[c].tiempo_reparacion : 0
